...
export default function OffshoreSegmentPage() {
  const segment = strategyData.segments.find(s => s.id === 'offshore');
  ...
  const relatedPriorities = strategyData.priorities.filter(p =>
    p.outcome === 'strategy-action' || p.outcome === 'faster-innovation'
  );
    ...
    <div className="space-y-6">
      ...
      <div className="card bg-slb-blue/5 border-slb-blue/20">
        <div className="flex items-center space-x-3 mb-3">
          <Anchor className="w-5 h-5 text-slb-blue" />
          <h3 className="text-sm font-medium text-slb-blue">Strategic Focus: Protect & Expand Offshore</h3>
        </div>
        <p className="text-sm font-light text-gray-700">
          Offshore is a key growth area in our 2030 ambition. Focus on deepwater and ultra-deepwater production chemistry,
          subsea production optimization, and offshore water treatment solutions. Leverage integrated solutions to differentiate
          from competitors.
        </p>
      </div>
        ...
        <div className="card">
          ...
          {segment.competitors && segment.competitors.length > 0 ? (
            ...
            <p className="text-sm font-light text-gray-500">
              Competitor data being compiled
            </p>
          )}
        </div>
      </div>
